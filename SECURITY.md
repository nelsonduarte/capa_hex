# Security

## Verifying the signature of a release

Published release tags are GPG-signed. The publisher's key:

- **Name**: Nelson Duarte (capa-language publisher)
- **Email**: nelson.duarte31@gmail.com
- **Fingerprint**:
  `6C1D 222D 491F B880 31E0  41A5 36CF B426 101A A24B`
- **Public key**: [`publisher.asc`](./publisher.asc) in this
  repository.

The full 40-character fingerprint without spaces:
`6C1D222D491FB88031E041A536CFB426101AA24B`. Use this exact
form in your `capa.toml`'s `verify_key` field (spaces and
colons are also accepted; the manifest parser normalises).

### Import the key

Out-of-band trust path (recommended): clone this repository,
inspect `publisher.asc`, then:

```bash
gpg --import publisher.asc
gpg --fingerprint nelson.duarte31@gmail.com
# Verify the printed fingerprint matches the one above.
```

Or fetch from a keyserver (less trustworthy; verify the
fingerprint independently):

```bash
gpg --recv-keys 6C1D222D491FB88031E041A536CFB426101AA24B
```

### Verify a tag manually

```bash
git clone https://github.com/nelsonduarte/capa_hex
cd capa_hex
git verify-tag v0.2.0
# Look for: "Good signature from "Nelson Duarte <...>" [ultimate]"
# and the fingerprint above.
```

### Verify automatically via `capa install`

Declare the fingerprint in your project's `capa.toml`:

```toml
[dependencies.capa_hex]
git = "https://github.com/nelsonduarte/capa_hex"
tag = "v0.2.0"
verify_key = "6C1D222D491FB88031E041A536CFB426101AA24B"
```

`capa install` runs `git verify-tag` against your keyring and
refuses to install when the signature is absent, invalid, or
from a different key. Details:
[docs/packages.md in the main Capa repo](https://github.com/nelsonduarte/capa-language/blob/main/docs/packages.md#sources-in-detail).

## Build provenance, and what gates a release

Each release also carries a SLSA L2 build-provenance attestation,
generated by CI through Sigstore with a keyless OIDC identity and
published to the Rekor transparency log. To check that the tarball you
downloaded was built by this repository's CI, from the commit the tag
names:

```bash
gh attestation verify capa_hex-v0.2.0.tar.gz --owner nelsonduarte
```

The signature above proves **who tagged it**; the attestation proves
**where it was built**. They are independent, and both are worth
running.

### The release guards

Before anything is built, attested or published, `.github/workflows/
release.yml` runs the shared release guards from the compiler
repository, pinned by commit SHA. Two things must hold:

1. **The tag matches the manifest version.** A sibling package once
   published v0.2.0 with its manifest still saying `0.1.0`, and nothing
   compared the two.
2. **The artefact builds from a clean location.** The tarball is
   extracted into a directory with no siblings, and the consumer flow
   is run against a **released** compiler, chosen from the `capa` floor
   this package declares. Another sibling package once shipped a
   release that was signed, attested and CI-green and did not compile
   for anyone who downloaded it, because the development machine's
   directory layout silently satisfied an undeclared import.

That second guard immediately found a real defect here: v0.1.0
declared `capa = ">=1.1.0"`, and capa 1.1.0 cannot compile
`example.capa` or the test suite at all. They self-import
`capa_hex.hex`, and in the extracted tarball the project root is named
`capa_hex-<tag>`, which that release cannot resolve. The floor is now
`>=1.18.1`, which is the oldest release the whole documented flow
actually passes on.

The bytes of the pinned guards are recorded in
`.github/guard-pins.sha256`; `tests/test_guard_pins.sh` re-fetches them
and refuses any difference, and `tests/test_release_wiring.sh` checks
that the workflow calls them in a way that actually gates the release,
and that the flow it hands them names **every** top-level module (see
[the ceiling note](README.md#audit-claim): the ceiling covers only the
import closure of the entry point it is given, so a module the flow
does not name is a module nothing checked).

`tests/test_wiring_mutations.sh` is the evidence that any of the above
bites. It applies thirteen edits to a scratch copy of this repository,
each of which leaves a release checking less than it claims, and
requires the wiring test to fail on every one. Three of the thirteen
came from an adversarial audit that applied them by hand and watched
the wiring test stay green; a fourth is the false compiler floor that
survived a by-the-book adoption of this template onto a sibling
package. A guard nobody has seen fail is a guard nobody has evidence
about.

**What the release does not gate.** `capa test --both`, which is what
backs the byte-identical-output claim, is absent from the consumer flow
because the clean room runs a *released* compiler binary and those
binaries carry no `wasmtime`: `--both` exits 2 there without running a
single test. Wasm parity is checked in development only.

## Security posture

- **Pure, zero-capability surface.** Every function in `hex.capa` is a
  total data transform over bytes and text. The library holds no
  authority: it cannot touch the filesystem, network, clock, randomness,
  or any other capability, and crosses no `unsafe`. Run
  `capa --manifest hex.capa` to see `declared_capabilities: []` and
  `has_unsafe: false` for every function. This bounds the blast radius
  if the dependency is ever compromised: a supply-chain attacker who
  owned this code still could not reach a sink from inside it.

- **Strict, total decode.** `decode` rejects any character outside
  `0-9a-fA-F`, and rejects an odd-length input, with a typed `HexError`.
  It never panics and never returns partial or wrong bytes. Both
  lowercase and uppercase input are accepted.

- **Hex is an encoding, not encryption.** It provides no confidentiality
  and no integrity. A hex string is not a secret and is not
  authenticated; do not treat it as either.

## Reporting a vulnerability

Use GitHub's private vulnerability reporting channel:

  https://github.com/nelsonduarte/capa_hex/security/advisories/new

Please include a reproducer if possible and allow up to 7 days
for an initial reply.
